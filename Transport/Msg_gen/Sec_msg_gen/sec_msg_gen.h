...
/** @def SEC_MSG_EE_TEMPERATURE
 *  @brief Message ID for the electronics embeded temperature secondary message
 */
#define SEC_MSG_EE_TEMPERATURE 				0x01
...
/** @fn 	HAL_StatusTypeDef send_sec_msg_ee_temperature(float temp, UART_HandleTypeDef * uart)
 * @brief 	Function used to send the electronics embeded temperature
 * @param 	temp is value to be sent
 * @param 	uart Uart strcture used to the communication with the Jetson Nano. If the cable used is the ST-Link, huart2
 * @return 	HAL_ERROR if something went wrong; HAL_OK if every went well
 */
